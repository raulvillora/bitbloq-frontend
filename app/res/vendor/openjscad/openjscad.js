...
// see the various methods below on how to change these
  ...
  this.solid = {
    ...
    color:   [1.0000,0.6275,0.0000,0.8627],        // default color
  };

  // Set up WebGL state
  var gl = GL.create({preserveDrawingBuffer: true}); // options
  this.gl = gl;
...
// the default options
  this.opts = {
    ...
    openJsCadPath: 'js/lib/openjscad/',
    useAsync: true,
    useSync:  true,
  };
// apply all options found
...
OpenJsCad.Processor.prototype = {
  createElements: function() {
    ...
    element = this.parametersdiv.querySelector("label#instantUpdateLabel");
    if (element === null) {
  ...
  setCurrentObject: function(objs) {
    ...
    var that = this;
    this.supportedFormatsForCurrentObject().forEach(
                      function(format) {
    ...
    if (this.formatDropdown.length > 0) {
  ...
  enableItems: function() {
    ...
    if (this.formatDropdown.options.length > 0) {
      ...
      this.formatDropdown.style.display = "none";
      this.generateOutputFileButton.style.display = "none";
      this.downloadOutputFileLink.style.display = "none";
    }
  ...
  getLibraryPath: function() {
    return this.opts[ 'openJsCadPath' ];
  },
  ...
  setColors: function(parameters) {
    if ( typeof( parameters ) !== 'undefined' ) {
      if ( typeof( parameters.color ) == 'string' ) {
        var values = parameters.color.split("/");
        for ( var i = 0; i < values.length; i++) {
          values[i] = values[i]/255;
        }
        if (values.length == 4) this.viewer.setSolidOptions({color: values});
      }
    }
  },
    ...
    // handle the results
      function(err, objs) {
        ...
        if(err) {
          ...
          that.setColors(parameters); // Z3D.jp special
          that.setCurrentObject(objs);
          var renderTime = Date.now();
          that.setStatus("Ready ("+(parseTime-that.startTime)+"ms/"+(renderTime-parseTime)+"ms)");
  ...
  rebuildSolidSync: function()
  {
    ...
    try {
      ...
      var parseTime  = Date.now();
      this.setColors(parameters); // Z3D.jp special
      this.setCurrentObject(obj);
      var renderTime = Date.now();
      this.setStatus("Ready ("+(parseTime-startTime)+"ms/"+(renderTime-parseTime)+"ms)");
  ...
  enableFormat: function(format) {
  // make sure the formats are initialized
    this.formatInfo("stla");

    if (!(this.formats[format] === undefined)) {
      this.formats[format].convertCSG = true;
      //this.formats[format].convertCAG = true;
    }
  },

  disableFormat: function(format) {
  // make sure the formats are initialized
    this.formatInfo("stla");

    if (!(this.formats[format] === undefined)) {
      this.formats[format].convertCSG = false;
      //this.formats[format].convertCAG = false;
    }
  },

  disableAllFormats: function() {
  // make sure the formats are initialized
    this.formatInfo("stla");
