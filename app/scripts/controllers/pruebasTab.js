...
/**
 * @ngdoc function
 * @name bitbloqApp.controller:pruebasTabCtrl
 * @description
 * # pruebasTab
 * Controller of the components list
 */
angular.module('bitbloqApp')
    .controller('pruebasTabCtrl', pruebasTabCtrl);

 function pruebasTabCtrl ( $rootScope, $scope, $timeout, $translate, $window, bloqsUtils, bloqs, bloqsApi,
        ...
        $scope.init = function () {
            ...
            currentProjectService.bloqs.varsBloq = bloqs.buildBloqWithContent($scope.currentProject.pruebas.vars, currentProjectService.componentsArray, bloqsApi.schemas, $scope.$field);
            currentProjectService.bloqs.setupBloq = bloqs.buildBloqWithContent($scope.currentProject.pruebas.setup, currentProjectService.componentsArray, bloqsApi.schemas);
            currentProjectService.bloqs.loopBloq = bloqs.buildBloqWithContent($scope.currentProject.pruebas.loop, currentProjectService.componentsArray, bloqsApi.schemas);
        ...
        $scope.initFreeBloqs = function () {
            ...
            if ($scope.currentProject.pruebas.freeBloqs && ($scope.currentProject.pruebas.freeBloqs.length > 0)) {
                for (i = 0; i < $scope.currentProject.pruebas.freeBloqs.length; i++) {
                    lastBottomConnector = null;
                    for (j = 0; j < $scope.currentProject.pruebas.freeBloqs[i].bloqGroup.length; j++) {
                        // $log.debug( $scope.currentProject.pruebas.freeBloqs[i].bloqGroup[j]);
                        tempBloq = bloqs.buildBloqWithContent($scope.currentProject.pruebas.freeBloqs[i].bloqGroup[j], currentProjectService.componentsArray, bloqsApi.schemas);
                        ...
                        if (lastBottomConnector) {
                            ...
                            tempBloq.$bloq[0].style.transform = 'translate(' + $scope.currentProject.pruebas.freeBloqs[i].position.left + 'px,' + $scope.currentProject.pruebas.freeBloqs[i].position.top + 'px)';
                        }
        ...
        $scope.setPruebasTab = function (tab) {
            $scope.pruebasTab = tab;
            if (tab === 'code') {
