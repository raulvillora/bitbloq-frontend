...
Blockly.JavaScript['pipette'] = function(block) {
	...
	JSONcode = JSONcode + '                    "groups" : [{ "' +type_pipette +'" :{ \n                        "from": ';
...
/**
 * Logic Blocks, adds condition tag to machine code
 
Blockly.JavaScript.expand_connected_blocks = function(block, condition) {
		var actualBlock = block;
		while(actualBlock) {
			actualBlock.condition = condition;
			actualBlock = actualBlock.getNextBlock();
		}
}; 

Blockly.JavaScript['controls_if'] = function (block) {
	var oldConditions = block.condition;
	var n = 0;
	while (block.getInputTargetBlock("IF" + n)) {
		var actualCondition = "(" + block.getInputTargetBlock("IF" + n).toString() + ")";
		Blockly.JavaScript.expand_connected_blocks(block.getInputTargetBlock("DO" + n), ((oldConditions) ?  oldConditions + " & " + actualCondition : actualCondition));
		n++;
		oldConditions = ((oldConditions) ? oldConditions + " & !" + actualCondition : "!" + actualCondition);
	}
	if (block.getInputTargetBlock("ELSE")) {
			Blockly.JavaScript.expand_connected_blocks(block.getInputTargetBlock("ELSE"), oldConditions);
	}	
	return "";
};
...
Blockly.JavaScript['controls_whileUntil'] = function(block) {
	var oldConditions = block.condition;
	if (block.getInputTargetBlock("BOOL")) {
		var actualCondition = "(" + block.getInputTargetBlock("BOOL").toString() + ")";
		Blockly.JavaScript.expand_connected_blocks(block.getInputTargetBlock("DO"), ((oldConditions) ?  oldConditions + " & " + actualCondition : actualCondition));
	}
	return "";
};*/
