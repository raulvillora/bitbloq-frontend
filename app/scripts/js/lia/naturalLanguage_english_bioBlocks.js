    ...
    if(block.getInputTargetBlock('destination') ){ //Get the block in the SOURCE input if exists
        ...
        if(isList){
            ...
            for(var i = 0; i < blockDestination.getFieldValue('contListOptionValue'); i++){
                ...
                if (currentBlock != null){
                    code= code +currentBlock.getFieldValue("containerName");
                    code = block.optionsDisplay_naturalLanguage(code, currentBlock); //Call the function optionsDisplay_ which it exists in each function block, with their own parameters.
        ...
        /*CASE only one block in destination*/
            ...
            if ( blockDestination.getInputTargetBlock('volume') || blockDestination.getInput('datareference') || blockDestination.getInput('singlewelladdrinput') || blockDestination.getInput('singleWell') || blockDestination.getInput('multipleWellAddrInput') || blockDestination.getInput('multiplewells') || blockDestination.getInput('gelcomposition') || blockDestination.getInput('valueagarose') || blockDestination.getInput('optionsCTMode') || blockDestination.getInput('optionsCTMode2') || blockDestination.getInput('steps')  ){
                ...
                code = block.optionsDisplay_naturalLanguage(code,blockDestination);  //Call the function optionsDisplay_ which it exists in each function block, with their own parameters.
...
Blockly.NaturalLanguage_english['continuous_flow'] = function(block) {
    ...
    switch (type) {
        case 1: { //one to one
            ...
            var source = fieldsValues.source;
            var sourceName = Blockly.NaturalLanguage_english.getContainerName(source);
            code += "from " + sourceName;

            var destination = fieldsValues.destination;
            var destinationNames = Blockly.NaturalLanguage_english.getContainerListNames(destination);
            var destinationRates = Blockly.NaturalLanguage_english.getContainerListRates(destination);
            ...
            var destination = fieldsValues.destination;
            var destinationName = Blockly.NaturalLanguage_english.getContainerName(destination);

            var source = fieldsValues.source;
            var sourceNames = Blockly.NaturalLanguage_english.getContainerListNames(source);
            var sourceRates = Blockly.NaturalLanguage_english.getContainerListRates(source);

            for(var i=0; i < sourceNames.length; i++) {
                var nameI = sourceNames[i];
                var rateI = sourceRates[i];
                code += " from " + nameI + " to " + destinationName + " at a rate of " + rateI;

                if (i != sourceNames.length -1) { //not the last one
                    code += " AND ";
                }
            }
